...
struct TrackEnrichmentFallbackTests {
	@Test
	func 상세정보조회가일부실패할때_searchTracksUseCase를실행하면_실패한트랙은원본을유지하는지() async throws {
		// given
		...
		let useCase = SearchTracksUseCaseImpl(trackRepository: repository)
	...
	func 상세정보조회가일부실패할때_fetchSimilarTracksUseCase를실행하면_실패한트랙은원본을유지하는지() async throws {
		...
		let useCase = FetchSimilarTracksUseCaseImpl(trackRepository: repository)
