...
struct GetArtistTimelineUseCaseTests {
	...
	mutating func executeSuccess() async throws {
		...
		let artistInfo = Artist(name: "Test Artist", imageURL: nil, listeners: "123", tags: ["Rock"], bio: "bio")
		let albums = [
		...
		mockRepository.fetchArtistInfoResult = .success(artistInfo)
		...
		#expect(result.artist.tags == ["Rock"])
		#expect(mockRepository.fetchAlbumsCallCount == 1)
		#expect(mockRepository.fetchArtistInfoCallCount == 1)
		#expect(mockRepository.lastFetchAlbumsArtist?.name == "Test Artist")
		#expect(mockRepository.lastFetchArtistInfoArtist?.name == "Test Artist")
	}
	...
	mutating func executeSortsAlbumsByReleaseDate() async throws {
		// Given
		let artist = Artist(name: "Test Artist", imageURL: nil)
		mockRepository.fetchArtistInfoResult = .success(artist)
		...
		#expect(result.albums[0].title == "New Album") // 최신순
		#expect(result.albums[1].title == "Mid Album")
		#expect(result.albums[2].title == "Old Album")
	}
	...
	mutating func executeSortsNilReleaseDateToEnd() async throws {
		...
		// Then
		#expect(result.albums.count == 3)
		#expect(result.albums[0].title == "Album With Date")
		// 나머지 두 개는 순서 상관없이 뒤에 위치
		#expect(result.albums[1].releaseDate == nil)
		#expect(result.albums[2].releaseDate == nil)
	}
